...
export const allWorkouts = () => ({
  ...
  payload: {
    request: {
      url: "https://mefitappdb.azurewebsites.net/api/v1/workouts",
    },
  },
});
...
export const deleteWorkout = (workout) => {
  console.log(
    `${UserService.getUsername()} deletes the workout ${workout.name}`
  );
  return {
    type: DELETE_WORKOUT,
    payload: {
      workout,
      request: {
        url: `https://mefitappdb.azurewebsites.net/api/v1/workouts/${workout.id}`,
...
export const addWorkoutToProfile = (workout) => {
  console.log(
    `${UserService.getUsername()} added Workout to profile ${workout.name}`
  );
  return {
    ...
    payload: {
      workout,
      request: {
        url: `https://mefitappdb.azurewebsites.net/api/v1/profiles//profiles/${UserService.getUsername()}`,
